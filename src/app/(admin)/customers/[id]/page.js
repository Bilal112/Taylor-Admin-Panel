...
const MEASUREMENT_FIELDS = [
  ...
  ['sleeveLength', 'Sleeve Length'], ['neck', 'Neck'], ['inseam', 'Inseam'],
...
export default function CustomerDetailPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setInfo({ name: cRes.data.data.name, phone: cRes.data.data.phone, email: cRes.data.data.email || '', address: cRes.data.data.address || '' });
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {MEASUREMENT_FIELDS.map(([k, l]) => (
            <div key={k}>
              ...
              <input type="number" step="0.5" className="input text-sm text-center"
                value={measurements[k] || ''} onChange={e => setMeasurements({ ...measurements, [k]: Number(e.target.value) })} />
            </div>
          ))}
        </div>
        <div>
          <label className="block text-xs text-gray-500 mb-1">Notes</label>
          <textarea className="input text-sm" rows={2} value={measurements.notes || ''} onChange={e => setMeasurements({ ...measurements, notes: e.target.value })} />
        </div>
        <button onClick={saveMeasurements} className="btn-primary text-sm">Save Measurements</button>
      </div>

      {/* Orders history */}
      <div className="card">
        <h2 className="font-semibold text-gray-700 mb-3">Order History ({orders.length})</h2>
