...
export default function OrdersPage() {
  ...
  const isDueTomorrowNotReady = (order) => {
    if (!order.promisedDate || ['ready', 'delivered', 'cancelled'].includes(order.status)) return false;
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => fetchOrders({ silent: true })} disabled={refreshing} className="btn-secondary text-sm" title="Refresh orders">
      ...
      <div className="card p-4 flex gap-3 flex-wrap">
        <form onSubmit={handleSearch} className="flex gap-2 flex-1 min-w-[200px]">
          <input className="input flex-1" placeholder="Search order number…"
            ...
            {orders.map(order => {
              ...
              return (
                <Link key={order._id} href={`/orders/${order._id}`}
                  className={clsx('card block space-y-2', dueTomorrow && 'bg-red-50 border-red-200')}>
                  ...
                  <div className="flex gap-1 flex-wrap">
                    {order.isRush && <span className="badge bg-red-500 text-white text-xs">RUSH</span>}
                    {dueTomorrow && <span className="badge bg-red-100 text-red-700 text-xs">⏰ Due Tomorrow</span>}
                  </div>
                  ...
                  {canSeeCustomer && order.customer?.name && (
                    <div className="text-xs text-gray-500">{order.customer.name} · {order.customer.phone}</div>
                  )}
                  <div className="flex items-center justify-between text-xs pt-1 border-t border-gray-50">
                    ...
                    {canSeeBalance && (
                      <span className={order.balanceDue > 0 ? 'text-red-600 font-medium' : 'text-green-600 font-medium'}>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-50">
                  {orders.map(order => {
                    const dueTomorrow = isDueTomorrowNotReady(order);
                    return (
                    <tr key={order._id} className={clsx(
                      ...
                      <td className="px-4 py-3 font-mono font-medium text-primary whitespace-nowrap">
                        ...
                        {dueTomorrow && <span className="badge bg-red-100 text-red-700 ml-2 text-xs">⏰ Due Tomorrow</span>}
                      </td>
                      ...
                      <td className="px-4 py-3 text-gray-700">
                        <div>
                          {(order.items?.length ? order.items : order.garmentType ? [order] : [])
                            .map((it, i) => `${it.garmentType}${it.quantity > 1 ? ` ×${it.quantity}` : ''}`)
                            .join(', ')}
                        </div>
                        {order.suitNo && <div className="text-xs text-gray-400">Suit No: {order.suitNo}</div>}
                      </td>
