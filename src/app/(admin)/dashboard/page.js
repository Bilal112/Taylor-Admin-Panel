        ...
        <div className="card border-l-4 border-red-500 bg-red-50">
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-semibold text-red-800 flex items-center gap-2 text-sm sm:text-base">
              <span>⏰</span> Due Tomorrow — Not Ready Yet ({stats.dueTomorrowCount})
            </h2>
          </div>
            ...
            {stats.dueTomorrowOrders?.map(o => (
              <Link key={o._id} href={`/orders/${o._id}`}
                ...
                <div className="min-w-0">
                  <span className="font-mono font-medium text-primary text-sm">{o.orderNumber}</span>
                  <span className="text-gray-500 text-sm ml-2">
                    {o.items?.map(it => it.garmentType).join(', ') || o.garmentType}
                  </span>
                  {isAdmin && o.customer?.name && (
                    <span className="text-gray-400 text-xs ml-2">· {o.customer.name}</span>
                  )}
                </div>
                <span className={`badge shrink-0 ${STATUS_COLORS[o.status] || 'bg-gray-100 text-gray-700'}`}>
